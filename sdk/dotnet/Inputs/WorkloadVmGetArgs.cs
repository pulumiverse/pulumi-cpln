// *** WARNING: this file was generated by pulumi-language-dotnet. ***
// *** Do not edit by hand unless you're certain you know what you are doing! ***

using System;
using System.Collections.Generic;
using System.Collections.Immutable;
using System.Threading.Tasks;
using Pulumi.Serialization;
using Pulumi;

namespace Pulumiverse.Cpln.Inputs
{

    public sealed class WorkloadVmGetArgs : global::Pulumi.ResourceArgs
    {
        [Input("accessCredentials")]
        private InputList<Inputs.WorkloadVmAccessCredentialGetArgs>? _accessCredentials;

        /// <summary>
        /// SSH public keys injected at runtime via the guest agent or config drive.
        /// </summary>
        public InputList<Inputs.WorkloadVmAccessCredentialGetArgs> AccessCredentials
        {
            get => _accessCredentials ?? (_accessCredentials = new InputList<Inputs.WorkloadVmAccessCredentialGetArgs>());
            set => _accessCredentials = value;
        }

        /// <summary>
        /// Boot disk configuration. When `Source` is omitted, `containers[0].image` is used as an OCI containerDisk.
        /// </summary>
        [Input("bootDisk")]
        public Input<Inputs.WorkloadVmBootDiskGetArgs>? BootDisk { get; set; }

        /// <summary>
        /// Guest clock configuration.
        /// </summary>
        [Input("clock")]
        public Input<Inputs.WorkloadVmClockGetArgs>? Clock { get; set; }

        /// <summary>
        /// Cloud-init configuration for the guest. Exactly one of `UserData`, `UserDataBase64`, or `UserDataSecret` must be specified.
        /// </summary>
        [Input("cloudInit")]
        public Input<Inputs.WorkloadVmCloudInitGetArgs>? CloudInit { get; set; }

        /// <summary>
        /// CPU topology visible to the guest. Cores are derived from `containers[0].cpu`.
        /// </summary>
        [Input("cpu")]
        public Input<Inputs.WorkloadVmCpuGetArgs>? Cpu { get; set; }

        /// <summary>
        /// Firmware configuration for the guest.
        /// </summary>
        [Input("firmware")]
        public Input<Inputs.WorkloadVmFirmwareGetArgs>? Firmware { get; set; }

        /// <summary>
        /// Guest operating system family. Drives the per-OS cloud-init payload. Valid values: `Linux`, `Windows`. Default: `Linux`.
        /// </summary>
        [Input("guestOs")]
        public Input<string>? GuestOs { get; set; }

        /// <summary>
        /// Hostname reported to the guest.
        /// </summary>
        [Input("hostname")]
        public Input<string>? Hostname { get; set; }

        [Input("networks")]
        private InputList<Inputs.WorkloadVmNetworkGetArgs>? _networks;

        /// <summary>
        /// Pod-network interfaces for the VM. Only a single network is supported.
        /// </summary>
        public InputList<Inputs.WorkloadVmNetworkGetArgs> Networks
        {
            get => _networks ?? (_networks = new InputList<Inputs.WorkloadVmNetworkGetArgs>());
            set => _networks = value;
        }

        /// <summary>
        /// KubeVirt RunStrategy. Use `Halted` to keep the pool defined but powered off. Valid values: `Always`, `RerunOnFailure`, `Manual`, `Halted`. Default: `Always`.
        /// </summary>
        [Input("runStrategy")]
        public Input<string>? RunStrategy { get; set; }

        /// <summary>
        /// Subdomain used by the guest for replica-to-replica addressing.
        /// </summary>
        [Input("subdomain")]
        public Input<string>? Subdomain { get; set; }

        public WorkloadVmGetArgs()
        {
        }
        public static new WorkloadVmGetArgs Empty => new WorkloadVmGetArgs();
    }
}
